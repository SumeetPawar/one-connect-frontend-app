...
// // ─── HABIT CARD ───────────────────────────────────────────────────────────────
// function HabitSelectCard({ habit, selected, disabled, onToggle }) {
//   const [expanded, setExpanded] = useState(false);
//   const tier = HABIT_TIERS[habit.id];
//   const barColor = tier ? TIER_META[tier].color : habit.color;
...
//   const [preview, setPreview]       = useState(null); // pack being previewed
...
//               const packs = ids.map(id => PACKS.find(p => p.id === id)).filter(Boolean);
//               return filterCat ? packs.filter(p => (GOAL_TAGS[p.id]||[]).includes(filterCat)) : packs;
//             };
...
//                     {PACK_META[preview.id].pill}
//                   </span>
//                 )}
//               </div>
//               {PACK_META[preview.id] && (
//                 <p style={{ fontSize:12, color:"rgba(255,255,255,0.3)", margin:"2px 0 0" }}>{PACK_META[preview.id].benefit}</p>
//               )}
//             </div>
//           </div>
...
//             <button onClick={() => { onStartDirect(preview.ids); setPreview(null); }} style={{
...
//             {/* balance hint */}
//             {(() => {
//               const hasBody      = selected.some(id => { const h = ALL_HABITS.find(x => x.id===id); return h && h.category==="Body"; });
//               const hasMind      = selected.some(id => { const h = ALL_HABITS.find(x => x.id===id); return h && h.category==="Mind"; });
//               const hasLifestyle = selected.some(id => { const h = ALL_HABITS.find(x => x.id===id); return h && h.category==="Lifestyle"; });
...
//               };
//               const sorted = [...selected].sort((a,b) => (RANK[a]||99) - (RANK[b]||99));
//               return (
...
//           .map(group => {
//             const habits = ALL_HABITS.filter(h => h.category === group.key);
...
// // ─── COMMITMENT SCREEN ────────────────────────────────────────────────────────
// function CommitmentScreen({ selectedIds, onCommit, onBack }) {
...
//             {habits.map(h => {
//               const tier = HABIT_TIERS[h.id];
//               const tc = tier ? TIER_META[tier].color : h.color;
//               return (
...
// // ─── COMMITTED SCREEN ────────────────────────────────────────────────────────
// function CommittedScreen({ selectedIds, onDone }) {
...
//   if (flow === "packs") return (
//     <PackPickerScreen
//       onSelectPack={ids => { setSelected(ids); setFlow("setup"); }}
//       onStartDirect={ids => { setSelected(ids); setFlow("commit"); }}
//       onCustom={() => { setSelected([]); setFlow("setup"); }}
//     />
//   );
//   if (flow === "setup") return <HabitSetupPage initialSelected={selected} onBack={() => setFlow("packs")} onComplete={ids => { setSelected(ids); setFlow("commit"); }}/>;
//   if (flow === "commit") return <CommitmentScreen selectedIds={selected} onBack={() => setFlow("setup")} onCommit={() => setFlow("committed")}/>;
//   if (flow === "committed") return <CommittedScreen selectedIds={selected} onDone={() => setFlow("tracking")}/>;

//   return <Day1Screen selected={selected} onRestart={() => setFlow("packs")} />;
// }

// function Day1Screen({ selected, onRestart }) {
