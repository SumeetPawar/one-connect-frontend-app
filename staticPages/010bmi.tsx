...
fl.rel = "stylesheet";
fl.href = "https://fonts.googleapis.com/css2?family=Figtree:wght@400;500;600;700;800;900&display=swap";
...
/* ─────────────────────────────────────────────────────────────
   SCAN VALUES  (latest scan)
───────────────────────────────────────────────────────────── */
const SCAN_VALUES = {
  weight:77.2, bmi:27.4, visceral:13, fat:27.2,
  muscle:42.1, water:46.2, bmr:1820, protein:17.8, metage:27, bone:3.2,
};

/* ─────────────────────────────────────────────────────────────
   COMPUTE IDEAL RANGES  from user profile
───────────────────────────────────────────────────────────── */
function computeRanges(age: number, gender: Gender, activity: Activity, height: number): Record<string, [number,number]> {
  ...
  const bmrBase  = isMale
    ? (10*SCAN_VALUES.weight)+(6.25*height)-(5*age)+5
    : (10*SCAN_VALUES.weight)+(6.25*height)-(5*age)-161;
...
───────────────────────────────────────────────────────────── */
function buildMetrics(ranges: Record<string, [number,number]>): Metric[] {
...
const ALL_TREND: TrendPt[] = [
  { label:"Feb 24", fat:22.1, muscle:36.8, water:54.2, visceral:7.2,  bone:2.9, protein:15.8, bmr:1740, metage:32, weight:74.2, bmi:23.6 },
  { label:"Apr 24", fat:21.8, muscle:37.2, water:54.8, visceral:7.0,  bone:2.9, protein:16.0, bmr:1752, metage:31, weight:73.8, bmi:23.5 },
  { label:"Jun 24", fat:21.4, muscle:38.0, water:53.9, visceral:7.4,  bone:3.0, protein:16.2, bmr:1765, metage:30, weight:73.5, bmi:23.4 },
  { label:"Aug 24", fat:21.0, muscle:38.6, water:53.2, visceral:7.8,  bone:3.0, protein:16.4, bmr:1778, metage:30, weight:73.1, bmi:23.3 },
  { label:"Oct 24", fat:22.4, muscle:38.2, water:51.8, visceral:8.6,  bone:3.1, protein:16.6, bmr:1790, metage:29, weight:73.6, bmi:23.5 },
  { label:"Nov 24", fat:23.6, muscle:38.0, water:50.4, visceral:9.8,  bone:3.1, protein:16.9, bmr:1798, metage:28, weight:74.0, bmi:23.6 },
  { label:"Dec 24", fat:24.8, muscle:37.8, water:49.2, visceral:10.8, bone:3.1, protein:17.0, bmr:1805, metage:28, weight:74.8, bmi:24.2 },
  { label:"Jan 7",  fat:25.6, muscle:41.4, water:48.0, visceral:11.4, bone:3.2, protein:17.2, bmr:1810, metage:28, weight:75.4, bmi:25.0 },
  { label:"Jan 21", fat:26.0, muscle:41.6, water:47.4, visceral:11.8, bone:3.2, protein:17.4, bmr:1814, metage:27, weight:75.8, bmi:25.8 },
  { label:"Feb 4",  fat:26.6, muscle:41.9, water:46.8, visceral:12.4, bone:3.2, protein:17.6, bmr:1818, metage:27, weight:76.4, bmi:26.6 },
  { label:"Feb 11", fat:26.9, muscle:42.0, water:46.5, visceral:12.7, bone:3.2, protein:17.7, bmr:1819, metage:27, weight:76.8, bmi:27.1 },
  { label:"Feb 18", fat:27.2, muscle:42.1, water:46.2, visceral:13.0, bone:3.2, protein:17.8, bmr:1820, metage:27, weight:77.2, bmi:27.4 },
];

const PERIOD_SLICES: Record<Period, TrendPt[]> = {
  "3M": ALL_TREND.slice(-6),
  "6M": ALL_TREND.slice(-8),
  "1Y": ALL_TREND.slice(-10),
  "All": ALL_TREND,
};
...
function RangeBar({ m }: { m: Metric }) {
  ...
  return (
    <div style={{ flex:1, minWidth:0 }}>
      <div style={{ position:"relative", height:4, borderRadius:100,
...
/* ─── History Graph ──────────────────────────────────────────── */
function HistoryGraph({ m }: { m: Metric }) {
  const [period, setPeriod] = useState<Period>("6M");
  const slice  = PERIOD_SLICES[period];
  const data   = slice.map(d => ({ label: d.label, value: d[m.trendKey]  }));
  const values = data.map(d => d.value);
  const domainMin = Math.min(...values, m.ideal[0]) - (m.ideal[0] - m.min) * 0.15;
  const domainMax = Math.max(...values, m.ideal[1]) + (m.max - m.ideal[1]) * 0.15;
...
function MetricRow({ m, last }: { m: Metric; last: boolean }) {
  ...
  const d3m      = PERIOD_SLICES["3M"].map(d => d[m.trendKey] );
  const diff3m   = parseFloat((d3m[d3m.length-1] - d3m[0]).toFixed(1));
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div style={{ width:80, flexShrink:0 }}>
            <div style={{ fontSize:13, fontWeight:500,
              color:"rgba(255,255,255,0.44)", lineHeight:1, marginBottom:4 }}>{m.label}</div>
...
export default function ScanReport() {
  ...
  // Profile state — defaults: male, 32, moderate, 177cm
  ...
  const ranges    = computeRanges(ageNum, gender, activity, heightNum);
  const METRICS   = buildMetrics(ranges);
    ...
    <div style={{ minHeight:"100vh", background:C.bg, color:C.text,
      ...
      <div style={{ position:"relative", zIndex:1 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", padding:"10px 16px 14px" }}>
            ...
            <div style={{ position:"absolute", left:"50%", transform:"translateX(-50%)",
              ...
              <div style={{ fontSize:15, fontWeight:600, letterSpacing:"-0.01em",
                color:"rgba(255,255,255,0.88)" }}>Body Composition</div>
              <div style={{ fontSize:11, color:C.dim }}>Feb 18, 2026</div>
            </div>
            <div style={{ marginLeft:"auto", textAlign:"right", flexShrink:0 }}>
              <div style={{ fontSize:10, color:C.dim, textTransform:"uppercase",
                letterSpacing:"0.05em" }}>Next scan</div>
        ...
        {(() => {
          ...
          const wLast         = Number(ALL_TREND[ALL_TREND.length-1][worst.trendKey]);
          const wPrev         = Number(ALL_TREND[ALL_TREND.length-2][worst.trendKey]);
          ...
          const best = METRICS.filter(m => m.value >= m.ideal[0] && m.value <= m.ideal[1])
            .sort((a,b) => {
              const d = (m) => Math.abs(m.value - (m.ideal[0]+m.ideal[1])/2) / ((m.ideal[1]-m.ideal[0])/2);
              return d(a) - d(b);
            })[0];
            ...
            <div style={{ padding:"12px 16px 20px", ...fade(0) }}>
              <div style={{
                ...
                <div style={{ padding:"18px 20px 0" }}>
                  <div style={{ fontSize:13, fontWeight:400, color:"rgba(255,255,255,0.36)",
                    letterSpacing:"0.01em" }}>Hello, Alex</div>
                </div>
                <div style={{ padding:"12px 20px 18px", display:"flex", alignItems:"center", gap:16 }}>
                  <div style={{ flexShrink:0, lineHeight:1 }}>
                    <span style={{ fontSize:64, fontWeight:700, letterSpacing:"-0.04em",
                      lineHeight:1, color:iColor }}>{goodCount}</span>
                    <span style={{ fontSize:18, fontWeight:400, letterSpacing:"-0.01em",
                      color:"rgba(255,255,255,0.18)", marginLeft:1 }}>/{METRICS.length}</span>
                  </div>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:22, fontWeight:700, color:"rgba(255,255,255,0.90)",
                      letterSpacing:"-0.03em", lineHeight:1.1, marginBottom:5 }}>{headline}</div>
                    <div style={{ fontSize:13, fontWeight:400, color:"rgba(255,255,255,0.36)",
                      lineHeight:1.4 }}>
                      {outOfRange.length === 0
        ...
        <div style={{ padding:"20px 16px 0", ...fade(1) }}>
          <div style={{ marginBottom:10, paddingLeft:2 }}>
            <span style={{ color:"rgba(255,255,255,0.32)", fontWeight:600, fontSize:11,
              letterSpacing:"0.05em", textTransform:"uppercase" }}>All Metrics</span>
          </div>
        ...
        <div style={{ padding:"20px 16px 0", ...fade(2) }}>
          <div style={{ fontSize:11, fontWeight:600, color:"rgba(255,255,255,0.28)",
            ...
            marginBottom:8, paddingLeft:2 }}>Health Profile</div>
          <button
      ...
      <>
        ...
        <div style={{
          ...
          <div onClick={() => setShowModal(false)}
            ...
            <div style={{ width:36, height:4, borderRadius:100,
          ...
          <div style={{ overflowY:"auto", flex:1, padding:"0 16px 8px" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
              ...
              <span style={{ fontSize:13, fontWeight:600, color:"rgba(255,255,255,0.60)",
                letterSpacing:"-0.01em" }}>Feb 18, 2026</span>
            </div>
            {[
                        ...
                        <div key={m.key} style={{
                          ...
                          <div style={{ display:"flex", alignItems:"center",
                            ...
                            <span style={{ fontSize:11, fontWeight:400,
                              ...
                              letterSpacing:"0.01em" }}>{displayUnit}</span>
                          </div>
                        </div>
                      );
                    })}
      ...
      <>
        ...
        <div style={{
          ...
          <div style={{ padding:"0 20px", display:"flex", flexDirection:"column", gap:20 }}>
            {/* Sex */}
            <div>
              <div style={{ fontSize:11, fontWeight:500, color:"rgba(255,255,255,0.36)",
                marginBottom:8 }}>Biological Sex</div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              <div>
                <div style={{ fontSize:11, fontWeight:500, color:"rgba(255,255,255,0.36)",
                  marginBottom:8 }}>Age</div>
                <input type="number" value={age} min={10} max={99}
                  onChange={e => setAge(e.target.value)}
                  style={{ width:"100%", height:44, background:"rgba(255,255,255,0.06)",
            ...
            <div>
              <div style={{ fontSize:11, fontWeight:500, color:"rgba(255,255,255,0.36)",
                marginBottom:8 }}>Activity Level</div>
