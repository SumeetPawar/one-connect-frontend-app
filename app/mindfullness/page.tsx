...
const TECHNIQUES: Technique[] = [
  {
    ...
    description: "Dr. Andrew Weil's natural tranquiliser. The calm kicks in around cycle 3–4 as the parasympathetic system activates.",
    accent: "#30A0C8",
    minCycles: 4,
    phases: [
  ...
  },
  {
    ...
    description: "Practiced by Navy SEALs and surgeons. CO₂ levels stabilise after 4–6 cycles — that's when mental stillness sets in.",
    accent: "#5E5CE6",
    minCycles: 4,
    phases: [
      { label: "Inhale", duration: 4, dir: "expand" },
  ...
  },
  {
    ...
    description: "5 breaths per minute synchronises heart rate variability. HRV effects require at least 5 sustained cycles to measure.",
    accent: "#34C759",
    minCycles: 5,
    phases: [
  ...
  },
  {
    ...
    description: "Sharp inhale, controlled exhale. You need 6–10 cycles to feel the oxygen shift — it's fast so more cycles required.",
    accent: "#FF9F0A",
    minCycles: 6,
    phases: [
...
const TIPS: TipSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: <Globe size={16} />,      title: "Screen stays on during sessions", detail: "The app keeps your screen awake automatically on Android so you're never interrupted. It releases the moment you end." },
      { icon: <RefreshCw size={16} />,  title: "Switch techniques anytime",       detail: "Each technique targets something different. Try 4·7·8 before sleep, Box Breathing before a meeting, Power Breath for a quick reset." },
    ],
  },
];
...
const CUES: Record<string, string[]> = {
  box: [
    ...
    "Let any thoughts drift past. Return to the box.",
  ...
  "478": [
    ...
    "Any thought that arrives — let it pass. Come back to the breath.",
  ...
  coherent: [
    ...
    "Let any thoughts drift through. You don't need to follow them.",
  ...
  power: [
    ...
    "Let any thoughts drift. Stay with the breath.",
    "You are exactly where you need to be.",
  ],
  guided: [
    "Let your body be completely still.",
    "Your only job right now is to listen.",
    "Let any thoughts drift past. Come back to the voice.",
...
const GENERAL_CUES = [
  ...
  "Let any thoughts drift past. Return to the breath.",
...
function useMindfulCues(active: boolean, techId: string) {
  ...
  const showNext = useCallback(() => {
    ...
    indexRef.current += 1;
    setVisible(true);
    // Fade out after 4s, next cue in 13s
...
const COMPLETION_MSGS = [
  { heading: "You showed up for yourself.",        sub: "Every session rewires how you respond to the world." },
  { heading: "This is who you are.",               sub: "People who breathe intentionally handle stress differently." },
  { heading: "Your nervous system thanks you.",    sub: "A few minutes of this changes your baseline — not just today." },
  { heading: "You chose stillness.",               sub: "In a world that rewards constant motion, this is an act of strength." },
  { heading: "You gave your mind a break.",        sub: "Most people never do. You just did." },
  { heading: "That counts.",                       sub: "Short, consistent sessions build something longer ones rarely do." },
  { heading: "You took care of yourself.",         sub: "That's not a small thing. It compounds." },
  { heading: "Your breath is always there.",       sub: "You just remembered how to use it." },
  { heading: "Stress leaves through the exhale.",  sub: "You just proved that to your body again." },
  { heading: "This is the habit forming.",         sub: "Identity shifts one session at a time." },
] as const;
...
export default function MeditationPage() {
    ...
    const tick = () => {
      ...
      if (elapsed >= dur) {
        ...
        if (navigator.vibrate) {
          if (nextLabel === "Inhale")
            navigator.vibrate([10, 30, 10, 30, 10]);        // rapid light taps — rising, urgent, draw in
          else if (nextLabel === "Hold")
            navigator.vibrate([80]);                         // single sustained — stillness, press and wait
          else if (nextLabel === "Exhale")
            navigator.vibrate([100, 60, 60, 40, 30, 20]);   // long decaying fade — release, let go
          else
  ...
  const stop = () => {
    ...
    if (secs > 10) {
      ...
      setSummary({ cycles, secs, tech, sessionCount: sessionCountRef.current, msgIdx: Math.floor(Math.random() * COMPLETION_MSGS.length) });
    }
    setTimeout(() => meditationRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 80);
  };
      ...
      {summary && (
        <>
          ...
          <h2 style={{
            ...
            color: "#fff", marginBottom: 4, lineHeight: 1.2, textAlign: "center",
            animation: "fadeUp 0.4s 0.3s ease both",
          }}>
            {COMPLETION_MSGS[summary.msgIdx].heading}
          </h2>
          <p style={{
            fontSize: 13, color: "rgba(255,255,255,0.28)", marginBottom: summary.cycles < summary.tech.minCycles ? 10 : 28,
            ...
            {summary.sessionCount % 5 === 0
              ...
              : COMPLETION_MSGS[summary.msgIdx].sub}
          </p>

          {summary.cycles < summary.tech.minCycles && (
            <div style={{
              marginBottom: 20, padding: "10px 16px", borderRadius: 12,
              background: `${summary.tech.accent}12`,
              border: `1px solid ${summary.tech.accent}30`,
              animation: "fadeUp 0.4s 0.38s ease both",
              maxWidth: 280, width: "100%",
            }}>
              <p style={{ fontSize: 12, color: `${summary.tech.accent}cc`, textAlign: "center", lineHeight: 1.5, letterSpacing: "-0.003em" }}>
                {summary.tech.minCycles} cycles is the sweet spot for {summary.tech.name}. Try going a little longer next time.
              </p>
            </div>
          )}
